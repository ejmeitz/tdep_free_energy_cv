...
def run_cv_free_energy(p : HeatCapFreeEnergyParams, paths : Paths,
    ...
    # Generate required input files for free energy
    # These are just dummy files so we can call anharmonic_free_energy
    # Normally needed to get U0, but we got that from interpolation
    md_dir = join(paths.basepath, "DUMMY_SIMULATION")
    os.mkdir(md_dir)
    if p.force_calc == "lammps":
        logging.info(f"Running dummy LAMMPS simulation")
        N_configs, N_atoms = run_dummy_lammps(p.temperatures[0], p.interp_settings.lds.structure_path,
    ...
    F_ph, F_3, F_4 = init_out_arrs(len(INTERP_TEMPS))
    ...
    for i,T in enumerate(INTERP_TEMPS):
        ...
        F_ph[i], F_3[i], F_4[i], _, _ = afe.parse_bulk_F_from_log(free_energy_dir_T)
    ...
    heat_cap_dir = join(paths.basepath, "HEAT_CAPACITY_RESULTS")
    os.mkdir(heat_cap_dir)
    for i, CENTER_TEMP in enumerate(p.temperatures):
        temp_str = temp_to_str(CENTER_TEMP)

        s = slice(i*p.fd_stencil_size, (i+1)*p.fd_stencil_size)
        ...
        np.savetxt(join(heat_cap_dir, f"heat_capacity_{temp_str}.txt"), np.column_stack([CENTER_TEMP, U0_deriv, F_ph_deriv, F3_deriv, F4_deriv, bulk_cv]),
                    fmt = "%.10f", header = "Cv / N kB decomposition\nTemperature U0_part F_ph_part F3_part F4_part Bulk_CV")

        np.savetxt(join(heat_cap_dir, f"free_energies_{temp_str}.txt"), np.column_stack([INTERP_TEMPS[s], U0_values[s], F_ph[s], F_3[s], F_4[s], F_total[s]]),
